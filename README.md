...
`make help` lists the available commands. The layout: the app lives in `src/delivery/` (`workflows.py`, `activities.py`, `worker.py`, and the service stubs under `stubs/`), the panel in `frontend/`, tests in `tests/`, and the Temporal dev server in `docker-compose.yml`.